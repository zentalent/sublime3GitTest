...
p,bundleIdentifier,serverpath,debugVersion,updateSVN,bundleVersion,packName,channel,sub_channel,buildChannel,buildDLL = argv
...
if startpacking() == 0 :
	...
	if debugVersion=='false':
		...
		if buildDLL== 'true' :
			...
			#rem 压缩
			os.system("java -jar %s\\apktool.jar b %s\\tmp -o %s\\tmp1.apk"\
			 % (apktool,APK_PATH,APK_PATH))
			print "SUCCESS 压缩"
		
			#rem resign
			os.system('call "%s" -storepass fanxing2016 -keystore %s\keystore.keystore -signedjar %s\%s %s\\tmp1.apk fxzjxx' % (jarsigner,apktool,APK_PATH,apkName,APK_PATH))
